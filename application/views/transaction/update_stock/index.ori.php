					...
					<table class="table table-striped table-valign-middle table-head-fixed text-nowrap m-0">
						...
						<tbody>
                            ...
                            foreach ($result as $row) :
                                ...
                                echo '<td>
                                            <span class="get-stock-awal get-stock-awal-' . $row->id . '" data-id="' . $row->id . '">Loading</span>
                                            <input type="hidden" name="stock_awal[]" id="input-stock-awal-' . $row->id . '">
                                    </td>';
	...
	<script>
        ...
        function get_stock_hilang() {
            $('.get-stock-hilang').each( function () {

                var id = $(this).data('id');
                var date = $('#date_now').val();

                $.ajax({
                    url: "<?=site_url('update_stock/get_data_stock_hilang');?>",
                    type: "post",
                    data: {
                        id: id,
                        date: date
                    ...
                    success: function (response) {
                        $('#input-stock-hilang-' + id).val(response);
                        count_stock_akhir();
                    }
                });
        ...
        function get_stock_rusak() {
            $('.get-stock-rusak').each( function () {

                var id = $(this).data('id');
                var date = $('#date_now').val();

                $.ajax({
                    url: "<?=site_url('update_stock/get_data_stock_rusak');?>",
                    type: "post",
                    data: {
                        id: id,
                        date: date
                    ...
                    success: function (response) {
                        $('#input-stock-rusak-' + id).val(response);
                        count_stock_akhir();
                    }
                });
        ...
        function get_stock_real() {
            $('.get-stock-real').each( function () {

                var id = $(this).data('id');
                var date = $('#date_now').val();

                $.ajax({
                    url: "<?=site_url('update_stock/get_data_stock_real');?>",
                    type: "post",
                    data: {
                        id: id,
                        date: date
                    ...
                    success: function (response) {
                        $('#input-stock-real-' + id).val(response);
                        count_stock_sisa(id);
                        count_stock_akhir();
                    }
                });
        ...
        function get_stock_awal() {
            $('.get-stock-awal').each( function () {

                var id = $(this).data('id');
                var date = $('#date_now').val();

                $.ajax({
                    url: "<?=site_url('update_stock/get_data_stock_awal');?>",
                    type: "post",
                    data: {
                        id: id,
                        date: date
                    ...
                    success: function (response) {
                        $('.get-stock-awal-' + id).html(response);

                        $('#input-stock-awal-' + id).val(response);
                        count_stock_akhir();
                    }
                });
        ...
        function count_stock_akhir() {
            ...
            $('.get-stock-akhir').each( function () {
                ...
                var akhir = awal + stock_in - stock_out - hilang - rusak;
